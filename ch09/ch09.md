...
## 9.3 CSS Module
> CSS를 불러와서 사용할 때 클래스 이름을 고유한 값, 즉 [파일이름] _ [클래스이름] _ [해시값] 형태로 자동으로 만들어서 컴포넌트 스타일 클래스 이름이 중첩되는 현상 방지
...
```
> 엘리먼트의 클래스에 highlighted rkqtdl true면 클래스 적용되고, false면 적용되지 않음.
